...
class VoiceAssistantGUI:
    ...
    def voice_assistant(self):
        vosk_model = vosk.Model("./vosk-model-en-in-0.5")
        ...
        while True:
            with sr.Microphone() as source:
                ...
                try:
                    ...
                    # Recognize speech using Vosk if internet is not available
                    if not self.check_internet():
                        ...
                        # Recognize speech using Google Speech Recognition if internet is available
                        query = self.recognize_online(audio)
                    ...
                    print("Error:", e)
                    self.engine.say("An error occurred. Please try again.")
                    self.engine.runAndWait()
    ...
    def close_current_window(self):
        # Use xdotool to get the title of the currently active window
        window_title = os.popen("xdotool getactivewindow getwindowname").read().strip()
        if window_title:
            # Use wmctrl to close the window with the retrieved title
    ...
    def process_query(self, query):
        if "the time" in query or "current time" in query:
            ...
            file_list = os.listdir()  # List files in the current directory
            print("Files in the directory:", file_list)
            self.engine.say("Here are the files in the directory:")
            for file in file_list:
                self.engine.say(file)
        elif "increase volume" in query:
            os.system("amixer -D pulse sset Master 10%+")  # Increase volume by 10%
            self.engine.say("volume increased 10%")
        elif "decrease volume" in query:
            os.system("amixer -D pulse sset Master 10%-")  # Decrease volume by 10%
            self.engine.say("volume decreased 10%")

        elif " set reminder" in query:
            reminder_text = query.replace("reminder", "").strip()
            print("Reminder set:", reminder_text)
            self.engine.say(f"Reminder set: {reminder_text}")

        if "CPU usage" in query:
            cpu_usage = psutil.cpu_percent()
            print("CPU Usage:", cpu_usage, "%")
            self.engine.say(f"The current CPU usage is {cpu_usage} percent.")

        elif "hello" in query:
            print("Hello! How can I assist you?")
            self.engine.say("Hello! How can I assist you?")
        elif "open calendar" in query:
            self.engine.say("opening calender")
            os.system("gnome-calendar")

        elif "open calculator" in query:
            self.engine.say("opening calculator")
        ...
        # Sleep for the specified duration
            ...
            self.engine.say("good morning, have a nice day")

        elif "good afternoon" in query.lower():
            self.engine.say("good afternoon, have a nice day")
